...
#pragma pack(1)
	...
	enum TIM_Channels : unsigned short
	{
	...
	enum TIM_JogMode : unsigned __int16
	{
	...
	enum TIM_ButtonsMode : unsigned __int16
	{
...
#pragma pack()
	...
	/// <seealso cref="TLI_GetDeviceListByTypeExt(char *receiveBuffer, DWORD sizeOfBuffer, int typeID)" />
	/// <seealso cref="TLI_GetDeviceListByTypesExt(char *receiveBuffer, DWORD sizeOfBuffer, int * typeIDs, int length)" />
	TCUBEINERTIALMOTOR_API short _cdecl TLI_GetDeviceInfo(char const * serialNo, TLI_DeviceInfo *info);
	...
	/// <returns> The error code (see \ref C_DLL_ERRORCODES_page "Error Codes") or zero if successful. </returns>
	TCUBEINERTIALMOTOR_API short __cdecl TIM_Home(TIM_Channels channel);
	...
	TCUBEINERTIALMOTOR_API WORD __cdecl TIM_GetLEDBrightness(char const * serialNo);
